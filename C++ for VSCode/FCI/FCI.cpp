...
Slater_det::Slater_det(const vector<int> &Orbital_ex)                                //将轨道信息写入k轨道中
{
    ...
    int nbeta = count(Orbital_ex.begin() + nOrb, Orbital_ex.end() + 2 * nOrb, 1);
    nelec = nalpha + nbeta;
...
inline int Slater_det::gamma(const int &I,const int &sigma)
{
    //轨道的I_n的gamma值
...
bool find(Slater_det &k1, Slater_det &k2, vector<int> &Num, vector<int> &index)
{
    ...
    for(int sigma=0;sigma<2;sigma++){
        ...
        for(int i=0;i<k1.nOrb;i++){
            if(k1.Orbital[i*k1.nOrb+sigma]!=k2.Orbital[i*k2.nOrb+sigma]){         //出现占据情况不同
                if(k1.Orbital[i*k1.nOrb+sigma]==1){                //k1该位置占据电子，k2未占据
                    index[0*k1.nOrb*2+count1*2+sigma]=i;
                    count1++;
                }
                else{                               //k2该位置占据电子，k1未占据
                    index[1*k1.nOrb*2+count2*2+sigma]=i;
                    count2++;
                }
...
//CI_Array用于存储组态
bool CI_new(int nelec_ex, int nOrb_ex, const double &MS, vector<int> &Orbital_ex, vector<Slater_det> &CI_Array)
{ 
    ...
    if(nelec_ex==0)
    {
        ...
        if (MS_ex != MS) //判断是否满足自旋条件，不满足则返回0
            return 1;
        else //满足自旋条件，返回1
        {
    ...
    {
        Orbital_ex[i - 1] = 1;
        flag = CI_new(nelec_ex - 1, i - 1, MS, Orbital_ex, CI_Array);
        Orbital_ex[i - 1] = 0;
    }
...
double CI::F_ij(Slater_det &k1, Slater_det &k2, const int sigma, const vector<int> &Num, const vector<int> &index)
{
    ...
    if(Num[sigma]==0){                                  //k1与k2的n自旋轨道相同
        ...
        int I = index[1 * dim * 2 + 0 * dim + sigma];        //k2的I_sigma分子轨道的n自旋轨道，位置占据电子，k1未占据
        int J = index[0 * dim * 2 + 0 * dim + sigma];        //k1的J_sigma分子轨道的n自旋轨道，位置占据电子，k2未占据
...
double CI::G1_ij(Slater_det &k1, Slater_det &k2, const int sigma, const vector<int> &Num, const vector<int> &index)
{
    ...
    if(Num[sigma]==0){                               //k1与k2的sigma自旋轨道相同
        ...
        int I=index[1*dim*2+0*dim+sigma];                    //k2的I分子轨道的sigma自旋轨道，位置占据电子，k1未占据
        int J=index[0*dim*2+0*dim+sigma];                    //k1的J分子轨道的sigma自旋轨道，位置占据电子，k2未占据
        ...
        int I = index[1 * dim * 2 + 0 * dim + sigma];        //k2的I分子轨道的sigma自旋轨道，位置占据电子，k1未占据
        int J = index[1 * dim * 2 + 1 * dim + sigma];        //k2的J分子轨道的sigma自旋轨道，位置占据电子，k2未占据
        int K = index[0 * dim * 2 + 0 * dim + sigma];        //k1的K分子轨道的sigma自旋轨道，位置占据电子，k1未占据
        int L = index[0 * dim * 2 + 1 * dim + sigma];        //k1的L分子轨道的sigma自旋轨道，位置占据电子，k2未占据
...
double CI::G2_ij(Slater_det &k1, Slater_det &k2, const vector<int> &Num, const vector<int> &index)//计算分子轨道i与分子轨道j关于hamilton算符的耦合项
{
    ...
    if(Num[0]==0&&Num[1]==0){                           //k1与k2的自旋轨道相同
        ...
        int I = index[1*dim*2+0*2+0];                  //1*nOrb*nOrb+0*nOrb+0,k2的I分子轨道的sigma自旋轨道，位置占据电子，k1未占据
        int J = index[0*dim*2+0*2+0];                            //0*nOrb*nOrb+0*nOrb+0,k1的J分子轨道的sigma自旋轨道，位置占据电子，k2未占据
        ...
        int I=index[1*dim*2+0*2+1];                        //k2的I分子轨道的sigma自旋轨道，位置占据电子，k1未占据
        int J=index[0*dim*2+0*2+1];                        //k1的J分子轨道的sigma自旋轨道，位置占据电子，k2未占据
        ...
        int I_alpha=index[1*dim*2+0*2+0];                  //k2的I分子轨道的alpha自旋轨道，位置占据电子，k1未占据
        int J_alpha=index[1*dim*2+1*2+0];                  //k2的J分子轨道的alpha自旋轨道，位置占据电子，k2未占据
        int I_beta=index[0*dim*2+0*2+1];                   //k1的I分子轨道的beta自旋轨道，位置占据电子，k1未占据
        int J_beta=index[0*dim*2+1*2+1];                   //k1的J分子轨道的beta自旋轨道，位置占据电子，k2未占据
        
        sum_int=k2.gamma(I_alpha,0)*k2.gamma(J_alpha,0)*k1.gamma(I_beta,1)*k1.gamma(J_beta,1)*g[I_alpha*dim3+J_alpha*dim2+I_beta*dim+J_beta];
    }
    else{
        sum_int=0;
    }
